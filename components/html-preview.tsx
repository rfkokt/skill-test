...
export default function HtmlPreview({
  ...
  const defaultTab = hasMultipleTestCases ? "expected-0" : "expected";
  ...
  const totalTabs = safeExpectedHtml.length + safeUserHtml.length;
  const gridColsClass = getGridColsClass(totalTabs);
        ...
        <Tabs defaultValue={defaultTab} className="w-full">
          ...
          >
            {safeExpectedHtml.map((_, index) => (
              ...
              >
                <span>
                  Expected {hasMultipleTestCases ? `Case ${index + 1}` : ""}
                </span>
              </TabsTrigger>
            ))}
            {safeUserHtml.map((_, index) => (
              <TabsTrigger
                key={`user-${index}`}
                value={`user-${index}`}
                className="flex items-center space-x-2 data-[state=active]:bg-neobrutal-softGreen data-[state=active]:text-neobrutal-softGreenText data-[state=active]:shadow-[inset_0px_0px_0px_2px_#333333]"
              >
                <span>
