...
import { problemsData } from "@/lib/problems"; // Import problemsData for ResumeTestModal logic
...
export default function CodingTestPlatform() {
  const {
    ...
    hasErrors,
    formatTime,
    setWebcamStream, // Correctly destructure setWebcamStream from the hook
    setSelectedEditorLanguage, // Correctly destructure setSelectedEditorLanguage from the hook
  } = useTestPlatform();
  ...
  if (currentScreen !== "selection" && !currentProblem) {
    ...
    setCurrentScreen("selection"); // Reset to selection screen
    setSelectedProblemId(""); // Clear selected problem
    return (
      <ProblemSelection
        ...
        <ResumeTestModal
          ...
          onConfirmResume={() => {
            ...
            if (problemToResume.requiresWebcam) {
              // Ensure webcam starts if required for resumed test
              const startWebcam = async () => {
                try {
                  const stream = await navigator.mediaDevices.getUserMedia({
                    video: true,
                  });
                  setWebcamStream(stream); // Use the destructured setWebcamStream
                } catch (err) {
            ...
            >
              ...
              {currentProblem.languages.map((lang) => (
                <TabsContent key={lang} value={lang} className="space-y-4">
                  {/* Code Editor */}
                  <div className="border-2 border-neobrutal-border rounded-lg bg-neobrutal-bg shadow-[2px_2px_0px_0px_#333333]">
                    <div className="flex h-[300px]">
                      ...
                      <div className="flex-1">
                        <textarea
                          ...
                          className="w-full h-full p-3 font-mono text-sm text-neobrutal-text bg-transparent border-none outline-none resize-none leading-6 overflow-y-auto"
                  ...
                  {currentProblem.id.startsWith("react-") &&
                    ...
                    userHtmlOutput && (
                      <HtmlPreview
                        htmlContent={userHtmlOutput}
                        title="Your Code's HTML Output"
                      />
                    )}
